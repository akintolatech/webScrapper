        ...
        try:
            ...
            if available_slots:
                ...
                if not appointment_type_selected:
                    print("Failed to select appointment type after retries.")
                    continue

                first_name_field = WebDriverWait(driver, 10).until(
                    EC.element_to_be_clickable((By.ID, "valFirstName"))
                )
                first_name_field.send_keys("John")

                last_name_field = WebDriverWait(driver, 10).until(
                    EC.element_to_be_clickable((By.ID, "valLastName"))
                )
                ...
                # Retry logic for solving CAPTCHA
                for attempt in range(max_retries):
                    try:
                        site_key = driver.find_element(By.CLASS_NAME, "g-recaptcha").get_attribute("data-sitekey")
                        current_url = driver.current_url
                        captcha_response = solve_2captcha(site_key, current_url)
                        ...
                        if "booking confirmed" in driver.page_source.lower():
                            ...
                            print("Booking attempt did not succeed.")
                    except Exception as e:
                        print(f"Retrying CAPTCHA ({attempt + 1}/{max_retries}) due to: {e}")
                        continue

                print("Failed to solve CAPTCHA after retries. Exiting...")
                return False
        ...
        for attempt in range(max_retries):
            try:
                ...
                if available_slots:
                    ...
                    if monitor_and_book_slot(driver):
                        return
                    else:
                        print("Failed to book slot after retries.")
                        return
                else:
